...
public final class Common {
	...
	public static boolean getShareList(HttpServletRequest request, int userId, int type, int startID, int pageSize) {
		...
		int ps = (pageSize != 0) ? pageSize : Config.PAGE_SIZE;
		String sql = "";
		...
		sql += " School.SchoolID,School.SchoolName,Users.UserID,Users.UserName,Users.Photo,"
				...
				+ "Share.Title,Share.Detail,Share.Longitude,Share.Latitude,Share.JoinCount,"
		...
		if (bRet) {
			try {
				...
				sql +=" order by Share.AddTime desc,Share.StartDate asc";
...
//				sql = Tools.getPageSql(sql, orderBy, pageSize);
				...
				SimpleDateFormat dotDateFormat = new SimpleDateFormat(
						"yyyy.MM.dd", new Locale("zh", "cn"));
				...
				nowDate = dateFormat.format(now) + " 00:00:00";
				...
				while (rsShareList.next()) {
					...
					s.setStartDate(dotDateFormat.format(timeTmp));

					timeTmp = rsShareList.getTimestamp("EndDate");
					String endDate = dotDateFormat.format(timeTmp);
					s.setEndDate(endDate);

					s.setLocation(rsShareList.getString("Location"));
					s.setTitle(rsShareList.getString("Title"));
					s.setDetail(rsShareList.getString("Detail"));
					s.setLongitude(rsShareList.getString("Longitude"));
					s.setLatitude(rsShareList.getString("Latitude"));
					...
					if(user.getUserId() == userId){
						...
						startDate += " 00:00:00";
						if(Tools.outOfDate(startDate, nowDate) == true){
							s.setJoinStatus(s.getJoinStatus() + 3);			//活动过期，不能再参加/取消参加
						}
						rsChk.close();
					}
					shareList.add(s);
