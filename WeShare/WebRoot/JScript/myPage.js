...
function checkASForm(){
	...
	if(dateCompare(startTime, endTime) == false){
		showMsg("活动开始时间不能大于活动结束时间！");
		return;
	}
	...
	var title = $id("activeTitle").value;
	if(title == "" || title == null){
		showMsg("标题还没写~");
		return;
	}
 ...
 */
function updateCnt(obj,sid,type,method,show){
	var bakClassName = obj.className;
	if(method != 2){
		obj.className = "clickDisable";
		...
		success: function(data){
			...
			if(data == "NG"){
				...
				if(type == 0){
					if(method == 0){
						$(txtId).html("取消参与");
						$id(txtId2).onclick = function(){updateCnt(obj,sid,0,1,true);};
						showMsg("参与成功！");
						//围观数据减1
						...
						$(txtId).html("参与");
						$id(txtId2).onclick = function(){updateCnt(obj,sid,0,0,true);};
						showMsg("成功退出该活动！");
						//恢复围观按钮的样式，附加onclick事件
						$(viewTxtId).removeClass().addClass("spanLink");
						$id(viewTxtId2).onclick = function(){updateCnt(obj,sid,1,0,true);};
					}
				}else if(type == 1){
					if(method == 0){
						$(txtId).html("取消围观");
						$id(txtId2).onclick = function(){updateCnt(obj,sid,1,1,true);};
						showMsg("围观成功！");
					}else{
						$(txtId).html("围观");
						$id(txtId2).onclick = function(){updateCnt(obj,sid,1,0,true);};
						if(show==true){
							showMsg("成功取消围观！");
						}
					}
...
function loadMore(){
	...
	if(reached == true){			//滚动条到达最底端
		if(loadTime == 1){			//只加载了一次，还可以自动加载一次
			...
			$(".loadingTip").html("<span id='loadClick' style='cursor: pointer;'>点击加载更多新鲜事...</span>");
